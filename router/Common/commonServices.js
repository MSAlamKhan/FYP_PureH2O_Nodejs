...
router.post('/addTransection', async (req, res) => {
    ...
    if (type == "credit") {
        ...
        console.log("my balance:", balanceResult[0].balance);
        var creditQuery = `UPDATE Tbl_wallet SET balance=${parseInt(balanceResult[0].balance) - parseInt(amount)} WHERE user_id=${userId}`
        const creditResult = await new Promise((resolve, reject) => {
            Connection.query(creditQuery, (err, result) => {
                if (err) reject(err);
                else resolve(result);
